/* --- Only this file is in scope. (If you find a 0day in sqlite you may of
 * course use it ;) --- */
...
int main(void) {
  ...
  rc = sqlite3_exec(db,
                    "CREATE TABLE IF NOT EXISTS entries(user INT, title TEXT, "
                    "language TEXT, content BLOB)",
                    NULL, 0, &errMsg);
  ...
  do {
    ...
    switch (c) {
    case '1': {
      if (!check_permissions(permission_create))
        continue;
    ...
    case '2': {
      if (!check_permissions(permission_update))
        continue;
    ...
    case '3': {
      if (!check_permissions(permission_view))
        continue;
    ...
    case '4': {
      if (!check_permissions(permission_list))
        continue;
    ...
    case '6':
      return 0;
    case '7': {
      if (!check_permissions(permission_root))
        continue;
...
void action_create() {
  ...
  rc = sqlite3_prepare_v2(
      db, "INSERT INTO entries(title, language, content) VALUES(?, ?, ?)", -1,
      &stmt, 0);
...
void action_update() {
  ...
  if (c == '2') {
    ...
    if (c == '1' || c == '3') {
      rc = sqlite3_prepare_v2(
          db, "UPDATE entries SET content=? WHERE title = ?", -1, &stmt, 0);
    } else if (c == '2') {
      rc = sqlite3_prepare_v2(
          db, "UPDATE entries SET content=HEX(?) WHERE title = ?", -1, &stmt,
          0);
    } else {
    ...
    rc = sqlite3_prepare_v2(db, "UPDATE entries SET language=? WHERE title = ?",
                            -1, &stmt, 0);
  }
  error_handle(SQLITE_OK);
...
void action_info() {
  sqlite3_stmt *stmt;
  rc = sqlite3_prepare_v2(
      db, "SELECT rowid, title, language, content FROM entries WHERE title = ?",
      -1, &stmt, 0);
  error_handle(SQLITE_OK);
...
void action_list() {
  sqlite3_stmt *stmt;
  rc = sqlite3_prepare_v2(
      db, "SELECT rowid, title, language, content FROM entries", -1, &stmt, 0);
  error_handle(SQLITE_OK);
...
void action_sys() { system("/usr/bin/cat flag"); }
